...
*Read this document [in English](architecture.md) — that is the canonical version.*
...
Esto es deliberadamente más simple que JitRL (cuya reponderación de logits
exige acceso al modelo) — el trade-off es menos elegante matemáticamente,
...
vuelve relativamente más agresivo el mismo `lr`.

Ten en cuenta además que `lo` es el suelo real del sistema: cuando los
motores rinden mejor que `target_error_rate`, el umbral baja hasta pegarse
a `lo` y se queda ahí — a partir de ese punto es `lo`, y no la tasa de
error objetivo, quien decide qué se acepta.

Si el error observado supera el objetivo, el umbral sube (más exigente,
escala más); si es menor, baja (menos escalamiento innecesario). Esto
sustituye a un umbral fijo puesto a mano, que es lo que hacen hoy los
routers comerciales revisados
